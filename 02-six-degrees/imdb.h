...
class imdb {
 ...
 private:
  ...
  void *binarySearch(const void *file, void *target, int (*compareFn)(void*, void*, const imdb*), int *a, int *b) const;
  film constructFilm(void *movieRecord) const;

  // Had to make these static because I couldn't pass them as an argument.
  // Had to add imdb* parameter because filmCompare needed to use a member function
  // Could I have done this better?
  static int actorCompare(void *x, void *y, const imdb *obj);
  static int filmCompare(void *x, void *y, const imdb *obj);
