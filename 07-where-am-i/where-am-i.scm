  ...
  (if 
    ...
    (append
      (apply append (map (lambda (c)
        (intersect (car circles) c)) (cdr circles))
      )
...
(define (rate-points points)
  (map (lambda (p)
        (list (distance-product p (remove p points)) p)
        )
    points
  )
)

(define (partition-generic pivot lst comp)
  (if (null? lst) '(() ())
      (let ((split-of-rest (partition-generic pivot (cdr lst) comp)))
	(if (< (comp (car lst) pivot) 0)
	    (list (cons (car lst) (car split-of-rest)) (cadr split-of-rest))
	    (list (car split-of-rest) (cons (car lst) (car (cdr split-of-rest))))))))


(define (quicksort-generic lst comp)
  (if (<= (length lst) 1) lst
      (let ((split (partition-generic (car lst) (cdr lst) comp)))
	(append (quicksort-generic (car split) comp) 
		(list (car lst)) 
		(quicksort-generic (cadr split) comp)))))


(define (sort-points rated_pts)
  (define (compare-rating rp1 rp2)
    (- (car rp1) (car rp2))
  )
  (quicksort-generic rated_pts compare-rating)
)
...
(define (average-point points)
  (let ((avg-pt 
    (make-pt (/ (apply + (map car points)) (length points)) 
             (/ (apply + (map cadr points)) (length points)))
