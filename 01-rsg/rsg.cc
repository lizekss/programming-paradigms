...
/* Accepts seed and reference to grammar map, recursively generates
   random sentences from non-terminal words. */
void generateRandomSentences(map<string, Definition> &grammar, string seed) {
  assert(grammar.find(seed) != grammar.end());

  Definition def = grammar[seed];
  Production prod = def.getRandomProduction();
  for (Production::iterator it = prod.begin(); it != prod.end(); it++) {
    string word = *it;
    if (word[0] == '<' && word[word.size() - 1] == '>') {
      generateRandomSentences(grammar, word);
    } else {
      //if (PUNCTUATION.find(word) == string::npos)
      // ^^ wanted to handle punctuation separately but kept 
      // failing one test
        cout << " ";
      cout << word;
    }
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  for (int i = 1; i <= NUM_VERSIONS; i++) {
    cout << "Version #" << i << ": --------------------------" << endl;
    generateRandomSentences(grammar, "<start>");
    cout << endl;
    cout << endl;
  }
