...
{ 
  pthread_mutex_lock(&bank->lockForWorkers);
  bank->workersLeft--;
  if (bank->workersLeft > 0) {
    pthread_cond_wait(&bank->startNextDay, &bank->lockForWorkers);
    pthread_mutex_unlock(&bank->lockForWorkers);
    return 0;
  } 
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      pthread_mutex_unlock(&bank->lockForWorkers);
      pthread_cond_broadcast(&bank->startNextDay);
      return -1;
  }
