          ...
          int numWorkers)
{
  ...
  pthread_mutex_init(&bank->lockForWorkers, NULL);
  pthread_cond_init(&bank->startNextDay, NULL);
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for (unsigned int branch = bank->numberBranches; branch > 0; branch--) {
      pthread_mutex_unlock(&(bank->branches[branch - 1].lock));
  }
